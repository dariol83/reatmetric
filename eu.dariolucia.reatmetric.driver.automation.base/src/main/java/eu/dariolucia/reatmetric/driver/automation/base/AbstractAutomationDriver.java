...
package eu.dariolucia.reatmetric.driver.automation.base;
...
import eu.dariolucia.reatmetric.driver.automation.base.common.AutomationConstants;
import eu.dariolucia.reatmetric.driver.automation.base.common.DataSubscriptionManager;
import eu.dariolucia.reatmetric.driver.automation.base.common.IScriptExecutor;
import eu.dariolucia.reatmetric.driver.automation.base.definition.AutomationConfiguration;
 ...
 */
public abstract class AbstractAutomationDriver implements IDriver, IActivityHandler {

    private static final Logger LOG = Logger.getLogger(AbstractAutomationDriver.class.getName());
    ...
    public AbstractAutomationDriver() {
        //
    }
    ...
    public void initialise(String name, String driverConfigurationDirectory, IServiceCoreContext context, ServiceCoreConfiguration coreConfiguration, IDriverListener subscriber) throws DriverException {
        try {
            ...
            this.executor = new ThreadPoolExecutor(configuration.getMaxParallelScripts(), // start with 1 thread
                    ...
                    (t) -> {
                        Thread toReturn = new Thread(t, this.name + " - Activity Handler Thread");
            ...
            InputStream is = getApiDataFileInputStream();
            apiData = readContents(is);
    ...
    protected abstract InputStream getApiDataFileInputStream();
    ...
    public List<String> getSupportedRoutes() {
        return Collections.singletonList(getAutomationRoute());
    }

    protected abstract String getAutomationRoute();
    ...
    public void executeActivity(ActivityInvocation activityInvocation) throws ActivityHandlingException {
        ...
        if (!getSupportedActivityTypes().contains(activityInvocation.getType())) {
    ...
    public boolean getRouteAvailability(String route) {
        return route.equals(getAutomationRoute());
    }
    ...
    private void execute(IActivityHandler.ActivityInvocation activityInvocation, IProcessingModel model) {
        try {
            ...
            String fileName = (String) activityInvocation.getArguments().get(AutomationConstants.ARGUMENT_FILE_NAME);
            if (fileName == null) {
    ...
    protected abstract IScriptExecutor buildScriptExecutor(ActivityInvocation activityInvocation, String fileName, String contents) throws ActivityHandlingException;
    ...
    protected static void announce(ActivityInvocation invocation, IProcessingModel model, Instant genTime, ActivityReportState reportState, Object result, ActivityOccurrenceState nextState) {
        model.reportActivityProgress(ActivityProgress.of(invocation.getActivityId(), invocation.getActivityOccurrenceId(), AbstractAutomationDriver.EXECUTION_STAGE, genTime, ActivityOccurrenceState.EXECUTION, genTime, reportState, nextState, result));
    }
    ...
    protected DataSubscriptionManager getDataSubscriptionManager() {
