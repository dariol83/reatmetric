...
In ReatMetric events can be reported by the drivers (so called _reported events_), or can be detected autonomously by the ReatMetric processing model,
by evaluating conditions upon change of parameters (so called _TM-based events_).
