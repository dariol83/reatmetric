...
the incoming route as part of its state. In the same way, when an activity occurrence is dispatched, the outgoing route
must be indicated. If the route is not available, then the dispatch of the activity occurrence fails.
...
- A route can be logically mapped to one or more logical connections.
...
For instance, _raw data_ can be used to distribute and store low level PDUs that are exchanged at connection level between
ReatMetric and the target devices. In the same way, it can be used to store changes of state or internal states of modules
